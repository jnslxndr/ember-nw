...
# Hamster Updates
# -------------
updateMessage = 'update Hamster to latest (Cakefile, package.json, portkey.json,
 config.coffee, generators/*)'
task 'tapas:update', updateMessage, (options) ->
  url = 'https://codeload.github.com/jens-a-e/hamster-brunch/zip/master'
  filesToUpdate = [
    'Cakefile'
    'package.json'
    'portkey.json'
  ...
  https.get url, (res) ->
    ...
    res.on('data', (chunk) ->
      ...
      filesToUpdate.forEach (file) ->
        targetFile = "hamster-brunch-master/#{file}"
        if /\/$/.test(file)
